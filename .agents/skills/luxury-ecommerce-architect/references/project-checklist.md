...
- [ ] Configure Arabic font (`next/font/google`: `Cairo` or `Tajawal`).
...
- [ ] **Sticky Glass Header**:
  ...
  - Theme toggle button.
  - Icon-only cart badge button (`w-9 h-9`) with count badge.
- [ ] **Mobile Bottom Navigation Dock**:
  - Fixed bottom bar on mobile (`sm:hidden`).
  - Nav items: الرئيسية, الكتالوج, تتبع الطلب, السلة, واتساب الدعم.
  - Active state golden glow indicator.
- [ ] **Luxury Footer**:
  - Brand statement, trust badges, customer links, tax/registration number, copyright.

---

## Phase 3: High-Converting Homepage
- [ ] **Hero Banner**:
  - High-res atmospheric background with blur gradient overlay.
  - Clear luxury value proposition in Arabic typography.
  - Shimmer call-to-action button linking to `/products`.
- [ ] **Trust Bar**:
  - 3 symmetric cards stacked on mobile (`grid-cols-1 max-w-sm mx-auto`).
  - 3 columns on desktop (`sm:grid-cols-3`).
  - Aligned icons, bold title, supportive subtitle.
