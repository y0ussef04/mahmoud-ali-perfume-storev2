    ...
    <article className="card group bg-white dark:bg-[#1C1A14] border border-[#E8E6E1] dark:border-[#2E2B22] rounded-xl sm:rounded-2xl p-2.5 sm:p-4 flex flex-col justify-between hover:shadow-md hover:border-[#C9A84C]/50 transition-all duration-200">
      <div>
        ...
        <Link href={`/products/${p.slug}`} className="block relative mb-2 sm:mb-3.5 rounded-lg sm:rounded-xl overflow-hidden bg-[#FAF9F5] dark:bg-[#151410]">
          ...
          <div className="absolute top-1.5 end-1.5 sm:top-2 sm:end-2 flex items-center gap-1 pointer-events-none">
            {allOut ? (
              <span className="bg-neutral-900/80 text-white text-[10px] sm:text-[11px] font-semibold rounded px-1.5 sm:px-2 py-0.5 backdrop-blur-xs">
                نفد المخزون
              </span>
            ) : discountPct > 0 ? (
              <span className="bg-red-600 text-white text-[10px] sm:text-[11px] font-bold rounded px-1.5 sm:px-2 py-0.5 num shadow-xs">
                خصم {discountPct}%
              </span>
            ) : null}
          </div>
        ...
        <div className="space-y-0.5 sm:space-y-1">
          ...
          <h3 className="text-xs sm:text-base font-semibold text-[#1A1814] dark:text-white line-clamp-1 group-hover:text-[#C9A84C] transition-colors duration-150">
            <Link href={`/products/${p.slug}`}>{p.name_ar}</Link>
          </h3>

          {p.name_en ? (
            <p className="text-[10px] sm:text-xs text-[#8C877D] dark:text-[#A09C94] line-clamp-1">
              {p.name_en}
            </p>
          ) : null}
        </div>
      ...
      <div className="mt-2.5 sm:mt-3.5 pt-2 sm:pt-3 border-t border-[#E8E6E1] dark:border-[#2E2B22] space-y-2 sm:space-y-2.5">
        {selectedVariant ? (
          <div className="flex items-baseline justify-between gap-1 flex-wrap">
            <div className="flex items-baseline gap-1 sm:gap-1.5 flex-wrap">
              ...
              {selectedVariant.compare_price && Number(selectedVariant.compare_price) > Number(selectedVariant.price) ? (
                <span className="text-[10px] sm:text-xs text-[#A09C94] line-through num">
        ...
        {allOut ? (
          <button
            type="button"
            disabled
            className="w-full bg-[#E8E6E1] dark:bg-[#2E2B22] text-[#8C877D] text-[11px] sm:text-xs font-semibold py-1.5 sm:py-2.5 rounded-lg sm:rounded-full min-h-[34px] sm:min-h-[40px] cursor-not-allowed flex items-center justify-center gap-1.5"
          >
            <ShoppingBag className="w-3.5 h-3.5 opacity-50" strokeWidth={1.5} />
            <span>غير متاح</span>
          </button>
        ) : (
          <button
            ...
            className="w-full group/btn relative overflow-hidden bg-gradient-to-r from-[#1A1814] to-[#2D2921] dark:from-[#C9A84C] dark:to-[#8B6914] text-white text-[11px] sm:text-xs font-semibold py-1.5 sm:py-2.5 rounded-lg sm:rounded-full transition-all duration-300 active:scale-[0.97] min-h-[34px] sm:min-h-[40px] flex items-center justify-center shadow-xs sm:shadow-md hover:shadow-lg hover:shadow-[#C9A84C]/20"
          >
            ...
            <div className="relative flex items-center justify-center gap-1.5 sm:gap-2">
              {addingState?.id === selectedVariant?.id ? (
                ...
                <span className="inline-flex items-center gap-1.5">
                  <ShoppingBag className="w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-300 group-hover/btn:scale-110" strokeWidth={1.5} />
